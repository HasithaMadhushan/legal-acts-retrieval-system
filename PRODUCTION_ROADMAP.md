...
### Phase 4 — Search quality & accuracy — ✅ DONE (2026-07-06)
- **F-004:** ✅ Fuzzy Act-title matching now ranks *all* ILIKE candidates by `difflib` similarity instead of taking whichever row Postgres/SQLite happened to return first (`.first()` with no `ORDER BY`, effectively arbitrary). Requires a minimum score (0.5) and a minimum lead over the runner-up (0.15); ambiguous ties are left unresolved with a "needs manual review" warning instead of silently guessing. See `reference_mapper._rank_partial_title_candidates`.
- **F-005:** ✅ Principal-enactment context is now tracked *sequentially* per reference in document order (`reference_mapper.map_references`, used by `document_processor.py`) instead of one Act-wide guess taken from the first citation found anywhere in the document. An omnibus Act that amends several different Acts in different sections now resolves each bare "Section N thereof" against the closest **preceding** explicit citation, switching correctly when the document moves on to a new target Act.
- **F-009:** ✅ `reference_extractor.py` now classifies relationships using a **sentence-bounded** window (nearest `.`/`;`/newline) instead of a fixed 160-character context, so a strong verb in an adjacent sentence can no longer be misattributed to an unrelated match. The generic bare-number patterns (`section N`, `subsection (N)`, `paragraph (a)`, `item N` -- ambiguous on their own, since they don't identify *which* Act) now require some operative/relationship phrase in the same sentence before creating a reference at all; Chapter/Schedule mentions are left ungated since they're identifying citations in their own right. Generic matches fully contained inside an already-captured structured match (e.g. the bare `paragraph (d)` inside "... is hereby amended by the repeal of paragraph (d) of subsection (1)") are no longer duplicated as a second reference. (Also fixed a related phrase-list gap: `"is hereby amended"`/`"is hereby inserted"` were missing from `RELATIONSHIP_PHRASES`, so the single most common statutory phrasing was falling through to the generic `REFERS_TO` classification.)
- **F-010:** ✅ `metadata_extractor.py` now recognizes `"... Ordinance"` titles (colonial-era Sri Lankan enactments, still amended today, e.g. the Poisons, Opium and Dangerous Drugs Ordinance) everywhere it previously only matched `"... Act"`, and the Act-number/date scan window was widened from 4,000 to 8,000 characters to cover Acts with a longer preamble.
- **F-013:** ✅ Postgres full-text search: a new migration (`20260706_01`) adds a generated, weighted (`title` > `category`/`source_name` > `raw_text`, and `heading` > `text` for sections), GIN-indexed `search_vector` `tsvector` column to `legal_acts` and `act_sections`. `search_service.py` uses `search_vector @@ plainto_tsquery(...)` for the expensive full-body-text match when running on Postgres (word-order-independent, stemmed -- verified manually against a real Postgres container: "establishments gaming" and "gaming establishment" both matched text containing "gaming establishments") and transparently falls back to the original `ILIKE` scan on SQLite, so local dev/tests need no changes. The migration is a no-op on SQLite (guarded on `dialect.name`), so `alembic upgrade head` still succeeds there.
- **Not done (deferred, needs a labeled corpus):** a seed gold dataset + evaluation-service regression run to *measure* the F-009/F-013 improvement on real Sri Lankan Act PDFs (the fixes above are covered by targeted unit tests with synthetic text, not a corpus-level precision/recall number).

> **Phase 4 end-to-end verification (2026-07-06):** ran the real migration against a Postgres container (`docker compose up db`) and confirmed the generated `search_vector` columns + GIN indexes exist (`\d legal_acts`, `\di`), then exercised `search_service.search()` directly against that database to confirm real full-text semantics (word-order-independent match and singular/plural stemming), not an accidental ILIKE fallback.
